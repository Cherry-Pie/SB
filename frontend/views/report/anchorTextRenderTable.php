        ...
        <div class="col-sm-12">
          <div class="pull-left">
            <div class="dataTables_info font-roboto-condensed text-primary" id="DataTables_Table_0_info">
              Anchor text / alt text <span class="current-items_count">1-10</span> из <?= $t; ?>
            </div>
          </div>
            ...
            <?php if (isset($value)): ?>
              ...
              <tr style='<?php if ($pagination > 1): ?>display: none; <?php endif; ?>' class="pag_page" page="<?=$pagination?>">
                ...
                <td class="content-cell">
                 <?php if($exept): ?>
                  <button type="button" class="btn btn-default btn-sm dropdown-toggle" data-toggle="collapse" data-target="#collapseTableForRow0<?= $i ?>">Domains <i class="fa fa-caret-down"></i></button>
                  <?= $key ?>
                  <?php else: ?>
                ...
                <td class="center">
                  <?php if ($exept): ?>
                    <a download="<?= $filter['url'] ?>-<?= $key ?>.csv" href="report/exporturls?search=<?= $key ?>&url=<?= $filter['url'] ?>&type=anchortext">
                      <span class="glyphicon glyphicon-download-alt text-warning"></span>
                    </a>
                  <?php endif ?>
                </td>
                    ...
                    <tr id="collapseTableForRow0<?= $i ?>" class="collapse odd sub-collapse-item">
                      ...
                      <td colspan="4">
                        <table border="0" cellpadding="0" cellspacing="0" class="table table-no-border sub-table" id="Row0<?= $i ?>CollapseSubtable">
                          ...
                          <?php $e = 0; ?>
                          <?php foreach ($value as $key_pk => $value_pk): ?>
                          <?php if (count($value_pk) == 0) continue; ?>                          
                            <tbody>
                              <!-- row 01 -->
                              <tr>
